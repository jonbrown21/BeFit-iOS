...
class LifestyleTableView: UITableViewController {
    //MARK: - Properties
    
    var tableData: [String]?
    weak var delegate: LifePickerControllerDelegate?
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let delegate = delegate {
            delegate.itemSelectedLifestyle(atRow: indexPath.row)
            dismiss(animated: true)
            
            let buttonTitle = String(format: "%lu", UInt64(indexPath.row))
            UserDefaults.standard.set(buttonTitle, forKey: "lifestyle-name")
