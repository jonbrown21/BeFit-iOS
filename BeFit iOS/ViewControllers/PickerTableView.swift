...
class PickerTableView: UITableViewController {
    ...
    weak var delegate: GenderPickerControllerDelegate?
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let delegate = delegate {
            delegate.itemSelectedGender(atRow: indexPath.row)
            dismiss(animated: true)
            
            let buttonTitle = String(format: "%lu", UInt64(indexPath.row))
            UserDefaults.standard.set(buttonTitle, forKey: "gender-name")
